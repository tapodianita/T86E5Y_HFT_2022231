...
{
  public class FlightsRepository : Repository<Flights>, IRepository<Flights>
  {
    public FlightsRepository(AirplaneDbContext ctx) : base(ctx)
    {
    }
    public override Flights Read(int id)
    {
      return ctx.Flight.FirstOrDefault(a => a.Id == id);
    }

    public override void Update(Flights item)
    {
